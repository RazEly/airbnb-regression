...
def add_h3_index(df: pd.DataFrame, h3_resolution: int) -> pd.DataFrame:
    ...
    if 'lat' in df.columns and 'long' in df.columns:
        df['h3_index'] = df.apply(lambda row: h3.latlng_to_cell(row['lat'], row['long'], h3_resolution), axis=1)
    return df
...
def get_closest_distance(
    ...
    distances = candidate_points.apply(lambda row: haversine(listing_loc, (row['lat'], row['long'])), axis=1)
    
    min_distance = distances.min()

    # If the closest point is still more than 100km away, return 100.0
    if min_distance > 100:
        return 100.0
