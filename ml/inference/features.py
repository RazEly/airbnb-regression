...
def prepare_features_for_model(
    ...
    # Combine all features (in metadata.json order - CRITICAL for model compatibility)
    final_features = {}
    ...
    if verbose:
        logger.info(
            f"Engineered {len(final_features)} features (city={geo_features.get('city_name')}, cluster_median={geo_features.get('cluster_median'):.2f})"
        )

        # Count None values
        none_count = sum(1 for v in final_features.values() if v is None)
        if none_count > 0:
            logger.info(f"Features with None values (will be imputed): {none_count}")
